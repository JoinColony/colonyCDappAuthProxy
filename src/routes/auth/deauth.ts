...
export const handleDeauthRoute = async (request: Request, response: Response) => {
  try {
    ...
    if (request.session.auth) {
      ...
      console.log(`Request to deauthenticate was successful ip: ${requestRemoteAddress} address: ${request.session?.auth?.address} cookie: ${request.headers.cookie}`)
    ...
    console.log(`Unauthentication user requested deauthentication ip: ${requestRemoteAddress} cookie: ${request.headers.cookie}`);
